  ...
  const computeValues = (data) => {
    if (data.length > 0) {
      const result = data.reduce(
        (result, current) => {
          if (current.cena < result.min.cena) {
            result.min = current;
          }
          if (current.cena > result.max.cena) {
            result.max = current;
          }
          return result;
        },
                    ...
                    <Table sx={{ border: '1px solid #ccc' }}>
                      ...
                      <TableBody>
                          <TableRow sx={{color: '#e7dfdd'}}>
                            <TableCell sx={{ border: '1px solid #ccc', color: '#e7dfdd' }}>{minMaxValues.min.cena}</TableCell>
                            <TableCell sx={{ border: '1px solid #ccc', color: '#e7dfdd' }}>{minMaxValues.min.data}</TableCell>
                          </TableRow>
                          <TableRow sx={{color: '#e7dfdd'}}>
                          ...
                          <TableRow sx={{color: '#e7dfdd'}}>
                            <TableCell sx={{ border: '1px solid #ccc', color: '#e7dfdd' }}>{minMaxValues.max.cena}</TableCell>
                            <TableCell sx={{ border: '1px solid #ccc', color: '#e7dfdd' }}>{minMaxValues.max.data}</TableCell>
                          </TableRow>
                      </TableBody>
                    </Table>
                  </Box>
